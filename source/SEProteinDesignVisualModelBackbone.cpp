...
void SEProteinDesignVisualModelBackbone::display() {
    ...
    float* endpositionData = new float[3]();
    float* endpathNodeRadiusData = new float[1]();
    float* endcolorData = new float[4]();
    unsigned int* endcapData = new unsigned int[1]();
    unsigned int* endflagData = new unsigned int[1]();

    float* beginpositionData = new float[3]();
    float* beginpathNodeRadiusData = new float[1]();
    float* begincolorData = new float[4]();
    unsigned int* begincapData = new unsigned int[1]();
    unsigned int* beginflagData = new unsigned int[1]();

    // fill in the arrays

    SBPosition3 position = endHelix->getPosition();
    endpositionData[0] = (float)position.v[0].getValue();
    endpositionData[1] = (float)position.v[1].getValue();
    endpositionData[2] = (float)position.v[2].getValue();


    endpathNodeRadiusData[0] = (float)SBQuantity::length(SBQuantity::angstrom(0.4)).getValue();
    endcolorData[0] = 1.0f;
    endcolorData[1] = 1.0f;
    endcolorData[2] = 0.0f;
    endcolorData[3] = 1.0f;
    endcapData[0] = 0;
    endflagData[0] = endHelix->getInheritedFlags();


    position = beginHelix->getPosition();
    beginpositionData[0] = (float)position.v[0].getValue();
    beginpositionData[1] = (float)position.v[1].getValue();
    beginpositionData[2] = (float)position.v[2].getValue();


    beginpathNodeRadiusData[0] = (float)SBQuantity::length(SBQuantity::angstrom(0.4)).getValue();
    begincolorData[0] = 3.0f;
    begincolorData[1] = 0.0f;
    begincolorData[2] = 0.0f;
    begincolorData[3] = 3.0f;
    begincapData[0] = 0;
    beginflagData[0] = beginHelix->getInheritedFlags();


    // display

    SAMSON::displaySpheres(1, endpositionData, endpathNodeRadiusData, endcolorData, endflagData);
    SAMSON::displaySpheres(1, beginpositionData, beginpathNodeRadiusData, begincolorData, beginflagData);
...
// Carbon.getNodeIndex
    if (pathNodeCarbonAlphaList.empty()) return;
        ...
        pathNodeRadiusData[0] = (float)SBQuantity::length(SBQuantity::angstrom(0.25)).getValue();
        ...
        pathNodeRadiusData[1] = (float)SBQuantity::length(SBQuantity::angstrom(0.25)).getValue();

        nodeIndexData[1] = endHelix->getNodeIndex();
