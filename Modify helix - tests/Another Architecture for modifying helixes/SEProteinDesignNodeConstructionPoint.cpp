...
SBPosition3 SEProteinDesignNodeConstructionPoint::getPosition() const{
    return position;
}

void SEProteinDesignNodeConstructionPoint::setPosition(const SBPosition3& position){
    this->position = position;
}

void SEProteinDesignNodeConstructionPoint::addNode(SEProteinDesignNodeCarbonAlpha* pathNodeCarbonAlpha) {

	if (!pathNodeCarbonAlpha) return;
	//if (SAMSON::isHolding()) SAMSON::addUndoCommand(new AddNodeCommand(this,node));
	CarbonList.addReferenceTarget(pathNodeCarbonAlpha);

}

void SEProteinDesignNodeConstructionPoint::removeNode(SEProteinDesignNodeCarbonAlpha* pathNodeCarbonAlpha) {

	if (!pathNodeCarbonAlpha) return;
	//if (SAMSON::isHolding()) SAMSON::addUndoCommand(new RemoveNodeCommand(this,node));
	CarbonList.removeReferenceTarget(pathNodeCarbonAlpha);

}

void SEProteinDesignNodeConstructionPoint::setLink(SEProteinDesignNodeConstructionPoint* ptr) {
	LinkedPoint = ptr;
}

SEProteinDesignNodeConstructionPoint* SEProteinDesignNodeConstructionPoint::getLink() {
	return LinkedPoint;
}

SBPointerList<SEProteinDesignNodeCarbonAlpha>* SEProteinDesignNodeConstructionPoint::getList() {
	return &CarbonList;
}

void SEProteinDesignNodeConstructionPoint::eraseCarbonAlpha() {
	CarbonList.erase(CarbonList.begin(), CarbonList.end());
}